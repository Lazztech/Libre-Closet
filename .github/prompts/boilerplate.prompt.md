...
6. The default expectation is traditional server-side rendering. Progressive enhancements may be added via htmx or \_hyperscript where they provide clear value (e.g. partial page updates, swipe gestures, drag-and-drop). Avoid client-side logic that duplicates what the server can render. In the views htmx, \_hyperscript, tailwind, and daisyui are available. Please use standard daisyui components from their documentation and prefer \_hyperscript for any necessary frontend logic instead of vanillajs in script tags. If front end code is likely to be reused it may be extracted out to a js file to be called on, located in public/js/ but this requires explicit approval.
